...
from exp33b import config as cfg  # Configure caches before third-party imports.
...
from exp33b.wiener import build_wiener, private_step, make_optimizer
...
def load_data(download: bool = True):
    ...
    # all Exp33b-generated files remain under exp33b/.
    root = cfg.DATA_ROOT
    return (
...
def run(method: str):
    ...
    linear_lr, scale_mode = spec["linear_lr"], spec["scale_mode"]
    ...
    optimizer = make_optimizer(model, linear_lr)
    ...
    configuration.update({
        ...
        "linear_lr": linear_lr, "identity_lr": cfg.IDENTITY_LR, "scale_mode": scale_mode, "seed": seed, "epochs": epochs,
        "a_scale_matching": scale_mode == "rms_match",
    ...
    for epoch in range(1, epochs + 1):
        ...
        wiener, builder = build_wiener(model, seed, epoch, sigma, scale_mode)
        ...
        row = {
            "method": method, "linear_lr": linear_lr, "identity_lr": cfg.IDENTITY_LR, "scale_mode": scale_mode, "seed": seed, "epoch": epoch,
        ...
        print(f"{method} scale={scale_mode} linear_lr={linear_lr} seed={seed} epoch={epoch}: accuracy={accuracy:.4f} loss={row['train_loss']:.5f} logical_steps={batches}", flush=True)

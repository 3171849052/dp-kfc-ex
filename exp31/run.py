...
from exp31 import config as cfg  # Configure caches before third-party imports.
...
from exp22.model import initialize as initialize_backbone
from exp31.lora import inject_lora, parameter_metrics, selected_layers

def initialize(seed, device):
    # Initialize head and LoRA on the seeded CPU RNG identically on every GPU.
    return inject_lora(initialize_backbone(seed, "cpu")).to(device)
...
def run(method: str, damping: float):
    if (method, damping) not in [(m, d) for _, m, d in cfg.grid()]:
        raise ValueError((method, damping))
    ...
    optimizer = torch.optim.AdamW(
        (p for p in model.parameters() if p.requires_grad), lr=cfg.LEARNING_RATE, betas=cfg.BETAS,
    ...
    for epoch in range(1, epochs + 1):
        ...
        operator, builder = build_geometry(
            model, method, probes, seed, epoch,
            damping=damping,
        )
        ...
        row = {
            ...
            **parameter_metrics(model, initial_parameters),
        ...
        assert row["frozen_backbone_unchanged"]
        assert batches == 195
        rows.append(row)
...
def main():
    ...
    parser.add_argument("--damping", type=float, choices=cfg.DAMPING_VALUES)

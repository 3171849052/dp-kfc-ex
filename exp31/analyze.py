...
def analyze(results=cfg.RESULTS):
    ...
    for d in cfg.DAMPING_VALUES:
        ...
        paired.append(dict(damping=d, dp_adamw_accuracy=b, dp_kfc_accuracy=k,
                           dp_kfc_a_accuracy=a, delta_kfc_vs_adamw=k-b,
                           delta_kfc_a_vs_adamw=a-b, delta_kfc_a_vs_kfc=a-k))
    pd.DataFrame(paired).to_csv(results / 'paired.csv', index=False)
    for metric, suffix in [('test_accuracy', 'accuracy'), ('clip_fraction', 'clip_fraction'),
        ...
        ax.axhline(baseline[metric], linestyle='--', color='gray', label=cfg.LABELS['dp_adamw'])

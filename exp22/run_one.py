...
def run(method: str, seed: int, smoke: bool, output: Path, smoke_batches: int = cfg.SMOKE_LOGICAL_BATCHES):
    ...
    for epoch in range(1, epochs + 1):
        ...
        batches, max_temp, backward_calls = 0, 0, 0
        for x, y in loader:
            x, y = x.to(device), y.to(device)
        ...
        row = {
            ...
            "best_accuracy": max(accuracies), "accuracy_auc": float(sum(accuracies) / len(accuracies)),
            "noise_multiplier": sigma,
            ...
            "bk_cache_bytes": 0, "temporary_per_sample_grad_bytes": max_temp,
            "fallback_temporary_grad_bytes": 0, "cache_empty_after_step": True,

...
if [[ "$SMOKE" == 1 && "$OUTPUT_SET" == 0 ]]; then
  OUTPUT="${ROOT_DIR}/exp22/results/smoke"
  mkdir -p "$OUTPUT"
fi
export PYTHONDONTWRITEBYTECODE=1
export XDG_CACHE_HOME="$ROOT_DIR/exp22/.cache"
export MPLCONFIGDIR="$ROOT_DIR/exp22/.cache/matplotlib"
export CUDA_CACHE_PATH="$ROOT_DIR/exp22/.cache/cuda"
export TMPDIR="$ROOT_DIR/exp22/.cache/tmp"
...
for seed in "${seeds[@]}"; do
  ...
  for method in "${methods[@]}"; do
    ...
    conda run -n curve python -B "$ROOT_DIR/exp22/run_one.py" "${args[@]}"
  done
done

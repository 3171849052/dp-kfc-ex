"""Head-only Ghost/BK clipping, noise injection, and optimizer helpers."""
...
from .handlers import layernorm_per_example_gradient, tensor_bytes
...
class Clipper:
    """Per-example clipping without materializing a per-example weight gradient.

    Module discovery is intentionally restricted to trainable module-local
    parameters.  With Exp24's model this makes both ``linear_modules`` and the
    operator's production domain exactly ``{"head"}`` (and no LayerNorms).
    """
    ...
    def __init__(
        ...
        if self.pos_embed is not None and not self.pos_embed.requires_grad:
            self.pos_embed = None
        self.cls_token = getattr(model, "cls_token", None)
        if self.cls_token is not None and not self.cls_token.requires_grad:
            self.cls_token = None
        self.parameters = [parameter for parameter in model.parameters() if parameter.requires_grad]
        self.parameter_ids = {id(parameter) for parameter in self.parameters}
        handled = {
            id(parameter)
            for module in self.linear_modules.values()
            for parameter in module.parameters(recurse=False)
        }
        handled.update(
            id(parameter)
            for module in self.norm_modules.values()
            for parameter in module.parameters(recurse=False)
        )
        ...
        if not selected <= set(self.linear_modules):
            raise ValueError(f"operator layers not in trainable model: {sorted(selected - set(self.linear_modules))}")
    ...
    def _linear_norm_squared(z, b, tile=8):
        """Ghost norm for sum_t b_t^T z_t, without a [B,Dout,Din] tensor."""
    ...
    def _linear_aggregate(z, b, factors):
        """Vectorized clipped aggregate with bias taken from augmented z."""
        ...
        for name, module in modules.items():
            def forward_hook(module, args, output, name=name):
                ...
                output.register_hook(
                    lambda grad, name=name: backprops.__setitem__(name, grad.detach())
                )
    ...
    def _one_batch(self, x, y, aggregate, loss_fn: LossFunction | None = None):
        ...
        if self.pos_embed is not None and not position_backprops:
        ...
        for name, module in self.norm_modules.items():
            ...
            for value in grads.values():
                sq.add_(value.reshape(len(x), -1).square().sum(dim=1))
            layer_sq[name] = sq
        ...
        return {
            ...
            "norms": norms.detach(),
            "factors": factors,
            "layer_sq": {key: value.detach() for key, value in layer_sq.items()},
            "group_sq": {key: value.detach() for key, value in group_sq.items()},
            "bk_cache_bytes": cache_bytes,
            "temporary_per_sample_grad_bytes": temporary_bytes,
            "fallback_temporary_grad_bytes": 0,
            "cache_empty_after_step": not activations and not backprops and not position_backprops,
        }
    ...
    def _finish(
        ...
        stats = {
            ...
            "layer_strategies": {
                ...
                **({"cls_token": "identity_direct"} if self.cls_token is not None else {}),
                **({"pos_embed": "identity_direct"} if self.pos_embed is not None else {}),
            },
    ...
    def _aggregate_chunks(self, x, y, physical_batch_size, loss_fn):
        ...
        for start in range(0, len(x), physical_batch_size):
            ...
            part = self._one_batch(x[start:stop], y[start:stop], aggregate, loss_fn)
            ...
            max_fallback_temporary = max(max_fallback_temporary, part["fallback_temporary_grad_bytes"])
    ...
    def aggregate_logical(self, x, y, physical_batch_size, loss_fn: LossFunction | None = None):
    ...
    def step(self, optimizer, sigma: float, batch_size: int, generator, bound: float | None = None):
        ...
        for parameter in self.parameters:
            ...
            parameter.grad.add_(noise, alpha=sigma * bound).div_(batch_size)
        optimizer.step()

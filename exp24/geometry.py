"""Synthetic head-only A/G factors and KFC operators for Exp24."""
...
def affine_modules(model: nn.Module) -> dict[str, nn.Linear]:
    """Return only affine modules whose own parameters are trainable."""
...
def inverse_sqrt(covariance: torch.Tensor, damping: float = cfg.DAMPING) -> torch.Tensor:
    return matrix_function(covariance, 0.5, damping)


class AOnlyOperator:
    """A-only right preconditioner with Exp20 RMS matching."""

    def __init__(
        self,
        factors: dict[str, dict],
        power: float = cfg.A_POWER,
        damping: float = cfg.DAMPING,
    ):
        self.power = power
        ...
        for name, factor in factors.items():
            covariance = (factor["A"] + factor["A"].T) * 0.5
            eigenvalues, eigenvectors = torch.linalg.eigh(covariance.double())
            eigenvalues = eigenvalues.clamp_min(0)
            output_dimension = factor["output_dimension"]
            raw += output_dimension * (eigenvalues * (eigenvalues + damping).pow(-2 * power)).sum()
            reference += output_dimension * (eigenvalues * (eigenvalues + damping).pow(-1)).sum()
            self.data[name] = (
                (eigenvectors * (eigenvalues + damping).pow(-power)) @ eigenvectors.T
            ).to(factor["A"].dtype)
        ...
        self.moments = {
            "m_p": raw_value,
            "m_reference": reference_value,
            "scale_match": self.scale,
        }
        eig = torch.cat(spectra) * self.scale**2
        gain = torch.cat(gains) * self.scale
        quantiles = torch.tensor((0.1, 0.5, 0.9, 0.99), device=eig.device, dtype=eig.dtype)
        values = torch.cat((gain.quantile(quantiles), eig.quantile(quantiles))).cpu().tolist()
        self.diagnostics = {
            f"{prefix}_p{percentile}": value
            for prefix, offset in (("operator_gain", 0), ("transformed_eig", 4))
            for percentile, value in zip((10, 50, 90, 99), values[offset:offset + 4])
        }
        self.diagnostics["transformed_condition_proxy"] = (
            values[6] / values[4] if values[4] > 0 else float("inf")
        )
        self.operator_state_bytes = sum(
            tensor.numel() * tensor.element_size() for tensor in self.data.values()
        ) + 8
...
class FullKFACOperator:
    """Full Kronecker preconditioner U_G g U_A for the head only."""

    def __init__(self, factors: dict[str, dict], damping: float = cfg.DAMPING):
        self.damping = damping
        self.factors = factors
        self.data: dict[str, tuple[torch.Tensor, torch.Tensor]] = {}
        for name, factor in factors.items():
            self.data[name] = (
                inverse_sqrt(factor["G"], damping),
                inverse_sqrt(factor["A"], damping),
            )
...
def _pink_noise(
    ...
    frequencies = torch.fft.fftfreq(cfg.IMG_SIZE, device=device)
...
def synthetic_stream(
    ...
    batches: int = cfg.SYNTHETIC_BATCHES,
    batch_size: int = cfg.SYNTHETIC_BATCH_SIZE,
    alpha: float = cfg.SYNTHETIC_ALPHA,
) -> Iterable[torch.Tensor]:
    """Lazily yield deterministic batches from one epoch-local image RNG."""
...
def synthetic_label_stream(
    ...
    generator = torch.Generator(device=device).manual_seed(seed + 20000 + epoch)
    return [
        synthetic_labels(seed, epoch, device, size, num_classes, generator)
...
def _selected(
    ...
    if missing:
        raise ValueError(f"unknown/non-trainable Linear geometry layers: {sorted(missing)}")
    if not selected:
        raise ValueError("no trainable affine layers selected")
...
def build_a_operator(
    ...
    """Build A from frozen-backbone features and the trainable head input."""
...
def build_full_operator(
    ...
    """Build A/G using one summed synthetic-label loss per physical batch."""
    ...
    try:
        for logical_x in batches:
            ...
            labels = synthetic_labels(
                ...
                logical_x.device,
                len(logical_x),
                num_classes or cfg.NUM_CLASSES,
                label_generator,
            )
            for start in range(0, len(logical_x), cfg.SYNTHETIC_PHYSICAL_BATCH_SIZE):
                x = logical_x[start:start + cfg.SYNTHETIC_PHYSICAL_BATCH_SIZE]
                y = labels[start:start + cfg.SYNTHETIC_PHYSICAL_BATCH_SIZE]

...
from exp33 import config as cfg  # Configure caches before third-party imports.
...
from exp33.wiener import build_wiener, private_step
...
def run(method: str):
    damping = 0.1 if method == "dp_kfc_a_01" else None
    ...
    for epoch in range(1, epochs + 1):
        ...
        operator, wiener = None, None
        builder = {"operator_state_bytes": 0, "builder_samples": 0}
        if method == "dp_kfc_a_01":
            operator, builder = build_reference(model, seed, epoch, device)
        elif method.startswith("dp_wiener"):
            wiener, builder = build_wiener(model, seed, epoch, sigma, method.endswith("full"))
        ...
        if method == "dp_kfc_a_01":
            for suffix in ("p50", "p90", "p99", "max"):
                row["transformed_norm_" + suffix] = row.pop("raw_private_norm_" + suffix)

"""Synthetic-only Wiener-A shrinkage filters and post-noise optimizer boundary."""
import torch
...
class Wiener:
    def __init__(self, factors, tau2, gamma):
        assert 0 <= gamma <= 1
        self.gamma = gamma
        self.data = {}
        gains, effective, groups, effective_groups = [], [], {}, {}
        for name, factor in factors.items():
            va, ua = spectrum(factor['A'])
            gain = va / (va + tau2)
            effective_gain = (1 - gamma) + gamma * gain
            assert torch.isfinite(gain).all() and gain.min() >= 0 and gain.max() <= 1
            assert effective_gain.min() >= 1 - gamma - 1e-7
            assert effective_gain.max() <= 1 + 1e-7
            self.data[name] = (ua.float(), gain.float(), effective_gain.float())
            gains.append(gain.cpu())
            effective.append(effective_gain.cpu())
            groups.setdefault(_group(name), []).append(gain.cpu())
            effective_groups.setdefault(_group(name), []).append(effective_gain.cpu())
        self.diagnostics = {
            **describe('raw_wiener_gain', torch.cat(gains)),
            **describe('effective_gain', torch.cat(effective)), 'tau2': tau2,
        }
        self.diagnostics.update({f'group_raw_wiener_gain_{g}': torch.cat(v).mean().item() for g, v in groups.items()})
        self.diagnostics.update({f'group_effective_gain_{g}': torch.cat(v).mean().item() for g, v in effective_groups.items()})
    ...
    def transform(self, name, z):
        if self.gamma == 0:
            return z
        ua, _, effective_gain = self.data[name]
        return ((z @ ua) * effective_gain) @ ua.T

    @torch.no_grad()
    def apply(self, model):
        if self.gamma == 0:
            return {f'group_wiener_norm_ratio_{_group(name)}': 1.0 for name in self.data}
        groups = {}
...
def build_wiener(model, seed, epoch, sigma, gamma, batches=None, physical_batch_size=128):
    ...
    operator = Wiener(factors, tau2, gamma)
...
def make_optimizer(model):
    return torch.optim.AdamW(
        ...
        lr=cfg.LEARNING_RATE, betas=cfg.BETAS, eps=cfg.ADAM_EPS,

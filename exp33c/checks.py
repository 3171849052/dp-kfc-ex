...
from exp33c.run import initialize, data_transform, private_loader
from exp33c.wiener import Wiener, build_wiener, matrix, private_step, make_optimizer, diagnostic_parameter_groups
...
def main():
    ...
    expected = [f'wiener_a_gamma_{g:g}' for g in (0., .1, .25, .5, .75, 1.)]
    assert list(cfg.RUNS) == expected and len(cfg.grid()) == 6
    assert cfg.GAMMAS == (0., .1, .25, .5, .75, 1.)
    assert cfg.GPU_RUNS == {0: (expected[0], expected[4]), 1: (expected[1], expected[5]),
                            2: (expected[2],), 3: (expected[3],)}
    ...
    for gamma in cfg.GAMMAS:
        op, stats = build_wiener(model, 42, 1, sigma, gamma, batches=xs, physical_batch_size=2)
        ops[gamma] = op
        assert stats['builder_samples'] == 8 and stats['builder_logical_batches'] == 2 and stats['builder_noise_events'] == 0
        assert stats['effective_gain_min'] >= 1 - gamma - 1e-7
        assert stats['effective_gain_max'] <= 1 + 1e-7
        for name, zs in matrices.items():
            vals, vecs = torch.linalg.eigh(covariance[name]['A'])
            gain = vals.clamp_min(0) / (vals.clamp_min(0) + (sigma / 256)**2)
            ua, actual_gain, effective = op.data[name]
            torch.testing.assert_close(actual_gain, gain.float(), atol=1e-6, rtol=1e-5)
            torch.testing.assert_close(effective, ((1-gamma) + gamma*gain).float(), atol=1e-7, rtol=1e-6)
            for z in zs:
                z = z.float()
                expected_z = (((z.double() @ vecs) * ((1-gamma) + gamma*gain)) @ vecs.T).float()
                torch.testing.assert_close(op.transform(name, z), expected_z, atol=1e-6, rtol=1e-5)
                if gamma == 0:
                    assert op.transform(name, z) is z
                if gamma == 1:
                    # Exact Exp33 Wiener-A float32 execution order.
                    assert torch.equal(op.transform(name, z), ((z @ ua) * actual_gain) @ ua.T)
    repeated, _ = build_wiener(model, 42, 1, sigma, .5, batches=xs, physical_batch_size=2)
    next_epoch, _ = build_wiener(model, 42, 2, sigma, .5, batches=xs, physical_batch_size=2)
    for name in matrices:
        for a, b in zip(ops[.5].data[name], repeated.data[name]):
    ...
    for stat in ('mean', 'median', 'p10', 'p90', 'p99', 'min', 'max'):
        assert original_stats[f'wiener_gain_{stat}'] == ops[1.].diagnostics[f'raw_wiener_gain_{stat}']
    print('PASS: direct Exp33 builder, raw gains and gamma=1 transforms are bitwise equal', flush=True)
    ...
    for gamma in cfg.GAMMAS:
        ...
        for x in xs:
            ...
            if gamma == 0:
                assert stats['cos_raw_clean'] == stats['cos_filtered_clean']
                assert stats['nsr_raw'] == stats['nsr_filtered']
                assert stats['wiener_norm_ratio'] == 1.
            ...
            assert abs(stats['cos_filtered_clean'] - cosine.item()) < 1e-6
            assert abs(stats['nsr_filtered'] - nsr.item()) < 1e-5
            ref_opt.step()
            for p, q in zip(trained.parameters(), reference.parameters()):
        ...
        clipper.remove()
    print('PASS: six-run grid, 2/2/1/1 GPUs, protocol/RDP, synthetic clipping/covariance, gamma endpoints/bounds, epoch rebuild, fixed LR, post-noise filtering, real AdamW updates', flush=True)
    ...
    from exp33c.wiener import synthetic_stream
    ...
    from exp33c.analyze import analyze
        ...
        for name, spec in cfg.RUNS.items():
            ...
            for epoch in range(1, 6):
                row = dict(method=name, **spec, epoch=epoch, logical_steps=195, accountant_steps=epoch*195,
                           ...
                           wiener_norm_ratio=.8, nsr_filtered=.2, nsr_raw=.3, cos_raw_clean=.4,
                           cos_filtered_clean=.5, mean_clip_factor=.6, effective_gain_min=1-spec["gamma"],
                           effective_gain_max=1.)
                for metric in ('update_norm', 'relative_update_norm'):
                    row.update({f'{metric}_{g}': .01 for g in cfg.UPDATE_GROUPS})
                for metric in ('effective_gain',):
                    row.update({f'{metric}_{q}': 1. for q in ('mean', 'median', 'p10', 'p90', 'p99')})
                rows.append(row)
            pd.DataFrame(rows).to_csv(directory / 'metrics.csv', index=False)
        summary, paired = analyze(results)
        assert len(summary) == 6 and len(paired) == 6
        assert (summary.epoch == 5).all() and (summary.exp33_dp_adamw_accuracy == .9276).all()
        assert (paired.delta_accuracy_vs_gamma0 == 0).all()
    ...
    print('PASS: epoch-5 summary (6), paired (6), fixed reference, ten plots, incomplete-run rejection', flush=True)

...
所有 cache、TMP、Torch/HuggingFace/Triton/matplotlib 路径都位于 `exp34/.cache/`；日志在 `exp34/logs/`，正式结果在 `exp34/results/`。Exp22 config 的 ROOT 仅在当前 Python 进程内重定向以避免其 model 导入写入旧缓存路径，不修改原文件。

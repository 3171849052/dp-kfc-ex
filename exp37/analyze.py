"""Analyze all eleven completed runs; comparisons use final-epoch metrics."""
import sys
from pathlib import Path
sys.dont_write_bytecode = True
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from exp37.config import ROOT, RUNS, cfg
import json
import pandas as pd
import matplotlib
matplotlib.use('Agg')
...
def plot(summary, fields, destination, filename):
    ...
    for ax, field in zip(axes[0], fields):
        for family, label in (('trace_a', 'Trace-A'), ('trace_ag', 'Trace-AG')):
            frame = summary.loc[summary.family == family].sort_values('p')
            ax.plot(frame.p, frame[field], 'o-', label=label)
...
def main():
    ...
    plot(summary, ['scale_min', 'scale_p10', 'scale_p50', 'scale_p90', 'scale_max'],
         destination, 'scalar_gain_distribution_vs_p.png')

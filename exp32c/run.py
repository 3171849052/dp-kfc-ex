"""One fresh SqueezeNet run with Exp30/31 fixed-batch RDP accounting."""
...
from exp32c import config as cfg  # Configure caches before third-party imports.
...
from exp32c.geometry import build_from_batches, synthetic_stream
...
def load_data(download: bool = False):
    ...
    if download:
        raise ValueError("Exp32c only reads the existing exp32/data dataset")
    root = cfg.DATA_ROOT
    return (
...
def run(method: str, damping: float):
    ...
    configuration.update({
        "method": method, "optimizer": "torch.optim.Adam",
        ...
        "data_config": {"input_size": [3, cfg.IMG_SIZE, cfg.IMG_SIZE],
                        ...
                        "mean": [0.485, 0.456, 0.406], "std": [0.229, 0.224, 0.225], "augmentation": None},
...
def main():
    ...
    if (args.method == "dp_adam") != (args.damping is None):
        parser.error("DP-Adam requires no damping; KFC methods require damping")

"""Decompose synthetic KFAC gains on full MNIST; run directly for 12 runs.

Oracle diagnostics and unnoised training metrics are analysis-only, not private
releases. privacy_valid describes only the inherited Exp6 training mechanism.
Full-Fisher Equil is not Kronecker-separable: its factor whitening is undefined
and recorded as NaN, never approximated by identity or fitted factors.

Comparisons: Full-Fisher Equil / KFAC-Equil: estimator structure;
KFAC-Diag / KFAC-Equil: coupling magnitude; KFAC-Equil / KFAC-Full-GM:
direction; KFAC-Full-GM / KFAC-Full-Raw: global operator scale.
"""
from pathlib import Path
import math
import sys
import time

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import pandas as pd
import torch
import torch.nn.functional as F
from torch.utils.data import DataLoader, Subset
from torchvision import datasets, transforms
from opacus import GradSampleModule
from opacus.accountants import RDPAccountant
from opacus.accountants.utils import get_noise_multiplier
from dp_kfac.models import SimpleCNN
from dp_kfac.optimizer import generate_pink_noise
from dp_kfac.recorder import KFACRecorder
from dp_kfac.covariance import compute_covariances
from dp_kfac.precondition import precondition_per_sample_gradients
from dp_kfac.privacy import (clip_and_noise_gradients,
                            _compute_per_sample_norms_squared, _compute_clip_factors)

METHODS = ("DP-SGD", "Full-Fisher Equil", "KFAC-Diag", "KFAC-Equil",
           "KFAC-Full-GM", "KFAC-Full-Raw")
SEEDS = (42, 7)
EPOCHS = 5
BATCH_SIZE = 256
PRECOND_STEPS = 10
PROBES = 8
DIAGNOSTIC_SAMPLES = 2048
DAMPING = 1e-3
LAYERS = ("conv1", "conv2", "fc1", "fc2")
GAIN_FIELDS = ("gain_min", "gain_p10", "gain_median", "gain_p90", "gain_max")


def pink_batches(batch_size, device):
    for _ in range(PRECOND_STEPS):
        yield (generate_pink_noise(batch_size, (1, 28, 28), device),
               torch.randint(0, 10, (batch_size,), device=device))


def rademacher(params, num_probes):
    return {p: torch.randint(0, 2, (p.numel(), num_probes), device=p.device)
            .to(p.dtype).mul_(2).sub_(1) for p in params}


def fisher_statistics(model, batches, device, probes=None):
    """Store only parameter blocks, B x K projections, and d x K accumulators."""
    params = [p for p in model.parameters() if p.requires_grad]
    diagonal = {p: torch.zeros(p.numel(), device=device) for p in params}
    products = {p: torch.zeros_like(probes[p]) for p in params} if probes is not None else {}
    if probes is not None:
        num_probes = next(iter(probes.values())).shape[1]
    count = 0
    model.train()
    for x, y in batches:
        model.zero_grad(set_to_none=True)
        # GSM uses sum reduction: these are unscaled per-example gradients.
        F.cross_entropy(model(x.to(device)), y.to(device), reduction="sum").backward()
        with torch.no_grad():
            if probes is not None:
                projection = torch.zeros(len(x), num_probes, device=device)
                for p in params:
                    projection.add_(p.grad_sample.flatten(1) @ probes[p])
            for p in params:
                block = p.grad_sample.flatten(1)
                diagonal[p].add_(block.square().sum(0))
                if probes is not None:
                    products[p].add_(block.T @ projection)
        count += len(x)
    model.zero_grad(set_to_none=True)
    diagonal = {p: (v / count).view_as(p) for p, v in diagonal.items()}
    equil = {p: (v / count).square().mean(1).sqrt().view_as(p)
             for p, v in products.items()}
    return diagonal, equil


@torch.no_grad()
def stabilized_scales(statistic):
    values = torch.cat([v.flatten() for v in statistic.values()])
    gamma = 1e-2 * values.median()
    scales = {p: (v + gamma).rsqrt() for p, v in statistic.items()}
    # One geometric mean over all coordinates, including every bias.
    log_mean = sum(v.log().sum() for v in scales.values()) / values.numel()
    normalizer = log_mean.exp()
    return {p: (v / normalizer).clamp(0.1, 10) for p, v in scales.items()}, normalizer.item()


def timestamp(device):
    if device.type == "cuda":
        torch.cuda.synchronize(device)
    return time.perf_counter()


@torch.no_grad()
def evaluate(model, loader, device):
    model.eval()
    loss, correct = 0., 0
    for x, y in loader:
        y = y.to(device)
        output = model(x.to(device))
        loss += F.cross_entropy(output, y, reduction="sum").item()
        correct += (output.argmax(1) == y).sum().item()
    return loss / len(loader.dataset), correct / len(loader.dataset)


def identity_factors(model):
    a, g = {}, {}
    for name in LAYERS:
        w = getattr(model._module, name).weight
        a[name] = torch.eye(w[0].numel() + 1, device=w.device, dtype=w.dtype)
        g[name] = torch.eye(w.shape[0], device=w.device, dtype=w.dtype)
    return a, g


def average_covariances(model, batches, device):
    """Sample-weighted factors, including an unequal final batch; no damping."""
    model.train()
    a, g = identity_factors(model)
    a = {k: torch.zeros_like(v, dtype=torch.float64) for k, v in a.items()}
    g = {k: torch.zeros_like(v, dtype=torch.float64) for k, v in g.items()}
    recorder = KFACRecorder(model)
    recorder.enable()
    count = 0
    for x, y in batches:
        model.zero_grad(set_to_none=True)
        F.cross_entropy(model(x.to(device)), y.to(device), reduction="sum").backward()
        with torch.no_grad():
            cov = compute_covariances(model,
                {k: v.double() for k, v in recorder.activations.items()},
                {k: v.double() for k, v in recorder.backprops.items()}, eps=0.0)
            for name in LAYERS:
                a[name].add_(cov.A[name], alpha=len(x))
                g[name].add_(cov.G[name], alpha=len(x))
        count += len(x)
        recorder.clear()
    recorder.remove()
    model.zero_grad(set_to_none=True)
    return ({k: v / count for k, v in a.items()},
            {k: v / count for k, v in g.items()})


@torch.no_grad()
def operator_gains(a, g):
    # Each (output, augmented input) is one trainable coordinate in SimpleCNN.
    return torch.cat([(g[k].norm(dim=0)[:, None] *
                       a[k].norm(dim=0)[None, :]).flatten() for k in LAYERS])


@torch.no_grad()
def kfac_preconditioner(model, cov_a, cov_g, method):
    a, g = {}, {}
    for covs, factors in ((cov_a, a), (cov_g, g)):
        for name, cov in covs.items():
            reg = cov + DAMPING * torch.eye(len(cov), device=cov.device, dtype=cov.dtype)
            if method == "KFAC-Diag":
                u = torch.diag(reg.diagonal().rsqrt())
            elif method == "KFAC-Equil":
                u = torch.diag(reg.norm(dim=1).rsqrt())
            else:
                eigenvalues, eigenvectors = torch.linalg.eigh(reg)
                u = (eigenvectors * eigenvalues.rsqrt()) @ eigenvectors.T
            factors[name] = u
    normalizer = operator_gains(a, g).log().mean().exp()
    if method != "KFAC-Full-Raw":
        for factors in (a, g):
            for u in factors.values():
                u.div_(normalizer.sqrt())
    dtype = next(model.parameters()).dtype
    return ({k: v.to(dtype) for k, v in a.items()},
            {k: v.to(dtype) for k, v in g.items()}, normalizer.item())


@torch.no_grad()
def gain_statistics(gains, before):
    """Quantiles describe the applied operator, after normalization/clamping."""
    quantiles = gains.quantile(gains.new_tensor([0., .1, .5, .9, 1.])).tolist()
    return dict(operator_geomean_before_norm=before, **dict(zip(GAIN_FIELDS, quantiles)))


def oracle(model, loader, a, g, device):
    private_a, private_g = average_covariances(model, loader, device)
    rows = []
    for name in LAYERS:
        ua, ug = a[name].double(), g[name].double()
        at = ua.T @ private_a[name] @ ua
        gt = ug @ private_g[name] @ ug.T
        abar = at / (at.trace() / len(at))
        gbar = gt / (gt.trace() / len(gt))
        n, m = len(abar), len(gbar)
        logs = []
        for c in (abar, gbar):
            ev = torch.linalg.eigvalsh(c)
            tol = len(c) * torch.finfo(c.dtype).eps * ev[-1]
            logs.append(float("inf") if ev[0] <= tol else (ev[-1].log() - ev[0].log()).item())
        rows.append(dict(layer=name,
            W_A=((abar - torch.eye(n, device=device, dtype=abar.dtype)).norm() / math.sqrt(n)).item(),
            W_G=((gbar - torch.eye(m, device=device, dtype=gbar.dtype)).norm() / math.sqrt(m)).item(),
            W_kron=(abar.square().sum() * gbar.square().sum() / (n*m) - 1).clamp(min=0).sqrt().item(),
            log_kappa_kron=sum(logs)))
    return rows


def run(method, seed, train, test, epochs, batch_size, sigma, device):
    torch.manual_seed(seed)
    model = GradSampleModule(SimpleCNN().to(device), loss_reduction="sum")
    params = [p for p in model.parameters() if p.requires_grad]
    optimizer = torch.optim.SGD(params, lr=.1, momentum=.9)
    loader = DataLoader(train, batch_size=batch_size, shuffle=True,
                        generator=torch.Generator().manual_seed(seed))
    test_loader = DataLoader(test, batch_size=batch_size,
                             generator=torch.Generator().manual_seed(seed))
    diagnostic_loader = DataLoader(Subset(train, range(DIAGNOSTIC_SAMPLES)),
        batch_size=batch_size, generator=torch.Generator().manual_seed(seed))
    accountant = RDPAccountant()
    rows, operators, whitening = [], [], []
    for epoch in range(1, epochs + 1):
        model.train()
        start = timestamp(device)
        if method == "DP-SGD":
            a, g = identity_factors(model)
            before = 1.
            gains = operator_gains(a, g)
        else:
            with torch.random.fork_rng(devices=[device.index] if device.type == "cuda" else []):
                torch.manual_seed(seed + 10000 + epoch)
                if method == "Full-Fisher Equil":
                    # Probe draws must not change the shared pink-batch sequence.
                    with torch.random.fork_rng(devices=[device.index] if device.type == "cuda" else []):
                        torch.manual_seed(seed + 30000 + epoch)
                        probes = rademacher(params, PROBES)
                    _, e = fisher_statistics(model, pink_batches(batch_size, device), device, probes)
                    scales, before = stabilized_scales(e)
                    gains = torch.cat([v.flatten() for v in scales.values()])
                else:
                    cov_a, cov_g = average_covariances(model, pink_batches(batch_size, device), device)
                    a, g, before = kfac_preconditioner(model, cov_a, cov_g, method)
                    gains = operator_gains(a, g)
        precond_seconds = timestamp(device) - start
        common = dict(method=method, seed=seed, epoch=epoch)
        operators.append(dict(**common, **gain_statistics(gains, before)))
        start = timestamp(device)
        loss_sum, clipped, factor_sum, count = 0., 0., 0., 0
        for x, y in loader:
            model.zero_grad(set_to_none=True)
            loss = F.cross_entropy(model(x.to(device)), y.to(device), reduction="sum")
            loss.backward()
            if method == "Full-Fisher Equil":
                with torch.no_grad():
                    for p in params:
                        p.grad_sample.mul_(scales[p])
            elif method != "DP-SGD":
                precondition_per_sample_gradients(model, a, g)
            norms_sq = _compute_per_sample_norms_squared(params, len(x), device)
            factors = _compute_clip_factors(norms_sq, 1.)
            clipped += (norms_sq.sqrt() > 1.).sum().item()
            factor_sum += factors.sum().item()
            clip_and_noise_gradients(model, sigma, 1., len(x))
            optimizer.step()
            accountant.step(noise_multiplier=sigma, sample_rate=batch_size / len(train))
            loss_sum += loss.item()
            count += len(x)
        train_seconds = timestamp(device) - start
        test_loss, accuracy = evaluate(model, test_loader, device)
        rows.append(dict(**common, privacy_valid=True, train_loss=loss_sum/count,
            test_loss=test_loss, test_accuracy=accuracy,
            epsilon_spent=accountant.get_epsilon(delta=1e-5), clip_fraction=clipped/count,
            mean_clip_factor=factor_sum/count, precond_seconds=precond_seconds,
            epoch_train_seconds=train_seconds))
        if method == "Full-Fisher Equil":
            analysis = [dict(layer=k, W_A=float("nan"), W_G=float("nan"),
                             W_kron=float("nan"), log_kappa_kron=float("nan")) for k in LAYERS]
        else:
            # Analysis cannot consume the RNG stream used by subsequent DP steps.
            with torch.random.fork_rng(devices=[device.index] if device.type == "cuda" else []):
                analysis = oracle(model, diagnostic_loader, a, g, device)
        whitening.extend(dict(**common, **d) for d in analysis)
        print(f"{method} seed={seed} epoch={epoch} accuracy={accuracy:.4f} "
              f"epsilon={rows[-1]['epsilon_spent']:.4f}", flush=True)
    return rows, operators, whitening


def plot_lines(ax, frame, metric):
    for i, method in enumerate(METHODS):
        part = frame[frame.method == method]
        if part.empty or part[metric].isna().all():
            continue
        for _, seed_rows in part.groupby("seed"):
            ax.plot(seed_rows.epoch, seed_rows[metric], color=f"C{i}", alpha=.3)
        mean = part.groupby("epoch")[metric].mean()
        ax.plot(mean.index, mean.values, marker="o", color=f"C{i}", label=method)
    ax.set(xlabel="Epoch", ylabel=metric, xticks=sorted(frame.epoch.unique()))
    ax.grid(alpha=.2)


def save_results(rows, operators, whitening, output):
    output.mkdir(parents=True, exist_ok=True)
    frame, ops, white = map(pd.DataFrame, (rows, operators, whitening))
    frame.to_csv(output / "metrics.csv", index=False)
    ops.to_csv(output / "operator_stats.csv", index=False)
    white.to_csv(output / "whitening.csv", index=False)
    final = frame.groupby(["method", "seed"], sort=False).tail(1)
    summary = final.groupby("method", sort=False).agg(
        mean_accuracy=("test_accuracy", "mean"), std_accuracy=("test_accuracy", "std"),
        mean_clipping=("clip_fraction", "mean"), mean_clip_factor=("mean_clip_factor", "mean"))
    timing = frame.groupby("method", sort=False).agg(
        mean_precond_seconds=("precond_seconds", "mean"),
        mean_epoch_train_seconds=("epoch_train_seconds", "mean"))
    summary.join(timing).to_csv(output / "summary.csv")
    for metric, filename in (("test_accuracy", "accuracy.png"), ("clip_fraction", "clipping.png")):
        fig, ax = plt.subplots(figsize=(9, 5))
        plot_lines(ax, frame, metric)
        ax.legend()
        fig.tight_layout()
        fig.savefig(output / filename, dpi=160)
        plt.close(fig)
    fig, axes = plt.subplots(2, 2, figsize=(12, 8))
    for ax, name in zip(axes.flat, LAYERS):
        plot_lines(ax, white[white.layer == name], "W_kron")
        ax.set_title(name)
    handles, labels = axes.flat[0].get_legend_handles_labels()
    if handles:
        axes.flat[0].legend(fontsize=8)
    fig.suptitle("Oracle whitening (Full-Fisher Equil: nonseparable, N/A)")
    fig.tight_layout()
    fig.savefig(output / "whitening_by_layer.png", dpi=160)
    plt.close(fig)
    fig, axes = plt.subplots(2, 3, figsize=(16, 8))
    for ax, method in zip(axes.flat, METHODS):
        part = ops[ops.method == method].groupby("epoch")[
            ["operator_geomean_before_norm", *GAIN_FIELDS]].mean()
        if not part.empty:
            ax.fill_between(part.index, part.gain_min, part.gain_max, alpha=.12, label="applied min–max")
            ax.fill_between(part.index, part.gain_p10, part.gain_p90, alpha=.3, label="applied p10–p90")
            ax.plot(part.index, part.gain_median, marker="o", label="applied median")
            ax.plot(part.index, part.operator_geomean_before_norm, linestyle="--", label="GM before normalization")
            ax.legend(fontsize=8)
        ax.set(title=method, xlabel="Epoch", ylabel="Operator gain (seed mean)", yscale="log")
        ax.grid(alpha=.2)
    fig.tight_layout()
    fig.savefig(output / "operator_scale.png", dpi=160)
    plt.close(fig)


def main():
    torch.set_num_threads(4)
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((.1307,), (.3081,))])
    train = datasets.MNIST(ROOT / "exp1/data", train=True, download=True, transform=transform)
    test = datasets.MNIST(ROOT / "exp1/data", train=False, download=True, transform=transform)
    sigma = get_noise_multiplier(target_epsilon=1., target_delta=1e-5,
        sample_rate=BATCH_SIZE/len(train), steps=EPOCHS*math.ceil(len(train)/BATCH_SIZE), accountant="rdp")
    output = ROOT / "exp7/results"
    print(f"device={device}, noise_multiplier={sigma}, output={output}", flush=True)
    rows, operators, whitening = [], [], []
    for seed in SEEDS:
        for method in METHODS:
            run_rows, run_ops, run_white = run(method, seed, train, test, EPOCHS, BATCH_SIZE, sigma, device)
            rows.extend(run_rows)
            operators.extend(run_ops)
            whitening.extend(run_white)
            save_results(rows, operators, whitening, output)


if __name__ == "__main__":
    main()

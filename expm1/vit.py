"""Offline pretrained ViT-Tiny task bindings for ExpM1."""
...
from expm1 import REPO_ROOT, ROOT, configure_environment
...
# imported. Point that one import at ExpM1, then restore the shared config.
...
def prepare_checkpoint() -> Path:
    """Copy Exp30's existing checkpoint into the isolated ExpM1 cache."""
...
def pink_calibration(seed: int, epoch: int, device: torch.device | str):
    return data.pink_calibration("vit", seed, epoch, device)

...
# output directories.  Missing STL10 (or any other fixed input) stops here.
...
stl = data.stl10_public_dataset()
assert (len(mnist_train), len(mnist_test), len(fashion)) == (60_000, 10_000, 60_000)
assert (len(cifar_train), len(cifar_test), len(stl)) == (50_000, 10_000, 5_000)
assert len(mnist_train.classes) == len(fashion.classes) == 10
assert len(cifar_train.classes) == len(stl.classes) == 10

...
from exp13.operator import Operator as ReferenceOperator
from exp13.ghost import GhostNorm, ghost_aggregate
from exp14c import config as cfg
from exp14c.builders import synthetic_cache, build_preconditioner
from exp14c.operator import Operator
from exp14c.run_exp14c import initialize


@pytest.fixture(scope='module', params=cfg.ESTIMATORS)
def setup(request):
    ...
    with runtime(device):
        ...
        factors, _ = estimate(model, cache, request.param, seed=20043)
        yield model, factors, device, request.param


def test_half_exp14_regression(setup):
    _, factors, _, _ = setup
    actual, expected = Operator(factors, .5), ReferenceOperator(factors)
    assert actual.diagnostics['scale_match'] == 1.
    for name in factors:
...
def test_builder_rng_isolation(setup):
    model, _, device, estimator = setup
    cpu, cuda = torch.random.get_rng_state(), torch.cuda.get_rng_state_all()
    for beta in cfg.BETAS:
        build_preconditioner(model, estimator, beta, 42, 1, device, batches=1, batch_size=4)
...
def test_same_initialization(setup, seed):
    _, _, device, _ = setup
    reference = initialize(seed, device)
    for beta in cfg.BETAS:
...
@pytest.mark.parametrize('beta', cfg.BETAS)
def test_estimator_scale_invariant(setup, beta):
    _, factors, _, _ = setup
    stats = Operator(factors, beta).diagnostics
    assert stats['scale_match'] ** 2 * stats['predicted_second_moment_raw'] == pytest.approx(
        stats['predicted_second_moment_ref'], rel=1e-12)
    assert stats['predicted_rms_matched'] == pytest.approx(
        stats['predicted_second_moment_ref'] ** .5, rel=1e-12)

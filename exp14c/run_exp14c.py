"""KFLR / KFRA-block RMS matched MNIST training; --smoke is one zero-noise batch for each beta."""
...
from exp14c.builders import synthetic_cache, build_from_cache
...
from exp14c.analyze import save
from exp14c.geometry import diagnose
    ...
    with torch.random.fork_rng():
        ...
        for estimator in cfg.ESTIMATORS:
            for beta in cfg.BETAS:
                operator, _, _ = build_from_cache(model, estimator, beta, cache, 90000, 0)
                del operator
        ...
        operator, _, _ = build_from_cache(model, cfg.ESTIMATORS[0], cfg.BETAS[0], cache, 90000, 0)
...
def save_config(output, sigma, epochs, seeds, train_size, smoke):
    ...
    config = dict(dataset='MNIST', model='dp_kfac.models.SimpleCNN', betas=cfg.BETAS, estimators=cfg.ESTIMATORS,
        ...
        scale_matching='estimator-local synthetic curvature global RMS', scale_reference_beta=.5,
...
def run(estimator, beta, seed, train, test, sigma, epochs, device, smoke=False):
    ...
    for epoch in range(1, epochs + 1):
        ...
        operator, stats, curvature = build_from_cache(model, estimator, beta, cache, seed, epoch)
        build_seconds = timestamp(device) - start
        build_peak = torch.cuda.max_memory_allocated(device)
        geometry_start = timestamp(device)
        geometry.extend(diagnose(curvature, estimator, beta, seed, epoch))
        del curvature
        ...
        row = dict(estimator=estimator, beta=beta, accountant='RDP', seed=seed, epoch=epoch, test_accuracy=accuracy,
            ...
            **stats)
        rows.append(row)
        print(f'{estimator} beta={beta} seed={seed} epoch={epoch} accuracy={accuracy:.4f} '
...
def main():
    ...
    with runtime(device):  # Deterministic settings and CUDA/cuBLAS warmup before timing.
        ...
        for seed in seeds:
            for estimator in cfg.ESTIMATORS:
                for beta in cfg.BETAS:
                    run_rows, run_geometry = run(estimator, beta, seed, train, test, sigma, epochs, device, args.smoke)

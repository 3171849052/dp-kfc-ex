...
sys.path.insert(0, str(ROOT / "exp3"))
from wiener_residual import WienerResidual, interval_ends

METHODS = ("Wiener-Residual-AG", "Wiener-Residual-AOnly")


def run(method, seed, train, test_loader, diagnostic_loader, epochs, batch_size, sigma, device, smoke=False):
    ...
    sensor = "wiener_filtered_grad"
    ends = interval_ends(len(loader))
    rows, feedback, wiener = [], [], []
    for epoch in range(1, epochs + 1):
        ...
        controller = WienerResidual(model, base_a, base_g, method, (sigma / batch_size)**2, smoke)
        ...
        for step, (x, y) in enumerate(loader, 1):
            ...
            loss_sum += loss.item()
            count += batch_size
            if step in ends or step == len(loader):
                wiener.extend(dict(method=method, seed=seed, epoch=epoch, interval=interval, **d)
                              for d in controller.interval_metrics())
            if step in ends:
        ...
        common = dict(method=method, seed=seed, epoch=epoch, privacy_valid=True,
...
def save_results(rows, feedback, wiener, output):
    ...
    wf = pd.DataFrame(wiener)
    wf.to_csv(output / "wiener_metrics.csv", index=False)
    ...
    for filename, fields in (
        ("wiener_feedback_quality.png", ("cos_raw", "cos_filtered", "NSR_raw", "NSR_filtered")),
        ("wiener_gain_distribution.png", ("wiener_gain_mean", "wiener_gain_median", "wiener_gain_p10", "wiener_gain_p90")),
    ):
        fig, axes = plt.subplots(2, 2, figsize=(13, 8))
        for ax, layer in zip(axes.flat, LAYERS):
            part = wf[wf.layer == layer].copy()
            part.epoch = part.epoch - 1 + part.interval / 5
            for metric, style in zip(fields, ("-", "--", ":", "-.")):
                plot_lines(ax, part, metric, style)
            ax.set_title(layer)
            ax.set_ylabel("Feedback quality" if "quality" in filename else "Wiener gain")
...
def main():
    ...
    for seed in seeds:
        for method in METHODS:
            r, f, w = run(method, seed, train, test_loader, diagnostic_loader, epochs, batch_size, sigma, device, args.smoke)
            rows.extend(r)
            feedback.extend(f)
            wiener.extend(w)
            save_results(rows, feedback, wiener, output)

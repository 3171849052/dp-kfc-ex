...
@contextmanager
def reference_constants(c):
    values = dict(MAX_GRAD_NORM=c, DELTA=cfg.DELTA, EPOCHS=cfg.EPOCHS,
                  ...
                  DAMPING=cfg.DAMPING, A_POWER=cfg.A_POWER, MAX_LENGTH=cfg.MAX_LENGTH)
...
def run_one(train, validation, tokenizer, c, lr, device):
    output = cfg.run_dir(c, lr)
    output.mkdir(parents=True, exist_ok=True)
    with reference_constants(c):
        ...
        row = reference.run_one(
            ...
            profile=False, profile_mode="practical", lr=lr, collect_diagnostics=True,
        )
    ...
    row.update(C=c, learning_rate=lr, noise_std=row["noise_multiplier"] * c,
               damping=cfg.DAMPING, a_power=cfg.A_POWER, collect_diagnostics=True)
    return row


def main():
    assert torch.cuda.is_available(), "Exp28 requires CUDA"
    cfg.RESULTS.mkdir(parents=True, exist_ok=True)
    # Protect completed and partial experiment results; no automatic resume.
    assert not (cfg.RESULTS / "summary.csv").exists(), "Use a fresh Exp28 results directory"
    assert not (cfg.RESULTS / "runs").exists(), "Use a fresh Exp28 results directory"
    with reference_constants(cfg.C_VALUES[0]):
    ...
    summaries = []
    for index, (c, lr) in enumerate(cfg.grid(), 1):
        print(f"[{index}/16] DP-KFC-A C={c:g} LR={lr:g} seed={cfg.SEED}", flush=True)
        summaries.append(run_one(train, validation, tokenizer, c, lr, torch.device("cuda")))

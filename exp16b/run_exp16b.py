...
from exp16b.preconditioner import Preconditioner


def configuration(q, seed=42, smoke=False, mode="nested"):
    extra = yaml.safe_load((ROOT / 'exp16b/configs/mnist.yaml').read_text())
    ...
    c['data']['root'] = str(ROOT / 'exp16b/data')
    c['algorithm'] = 'synthetic_two_stage'
    c['output']['root'] = str(ROOT / 'exp16b/results')
    if smoke:
        c['data']['batch_size'] = 4
...
def run(q, seed=42, smoke=False, mode="nested"):
    ...
    directory = ROOT / 'exp16b/results' / ('smoke' if smoke else 'formal') / f'{mode}_q{q:g}_seed{seed}'
...
def analyze(smoke=False):
    directory = ROOT / 'exp16b/results' / ('smoke' if smoke else 'formal')

# exp16b：clipping/noise 前的两阶段预条件

复用 exp15 的 MNIST + SimpleCNN、synthetic K-LBFGS、训练及隐私会计协议。
所有新代码、数据缓存和输出位于 exp16b；exp15 作为只读依赖。

## 与 exp16 的唯一区别

唯一算法性改动是 Hessian / K-LBFGS damping：

$$
\boxed{\lambda_H: 0.0316227766 \rightarrow 0.1}
$$

Fisher/KFAC damping 保持不变：

$$
\boxed{\lambda_F=0.001}
$$

memory=100、activation_decay=0.9、pair_decay=0.9、
synthetic_lookahead_lr=0.5 及其他训练参数全部沿用 exp16。
代码从 exp16 复制并改为 exp16b 路径；exp15 的 K-LBFGS 实现仅只读复用，
实际收到的 Kron_BFGS_H_epsilon 为 0.1。
...
Hessian 仍使用 exp15 的紧凑 LBFGS 谱分解及 factor_damping=0.1。
...
配置读取 exp16b/configs/mnist.yaml 和原 standalone base；
MNIST 缓存单独位于 exp16b/data，避免向旧实验目录写入数据。
...
```bash
PYTHONDONTWRITEBYTECODE=1 conda run -n curve python -m pytest exp16b/tests/test_exp16b.py -q -o cache_dir=exp16b/results/pytest_cache > exp16b/results/unit_tests.log 2>&1
PYTHONDONTWRITEBYTECODE=1 conda run -n curve python exp16b/run_exp16b.py --smoke > exp16b/results/smoke.log 2>&1
```
...
```bash
conda run -n curve bash exp16b/run_all.sh
```
...
```bash
conda run -n curve python exp16b/run_exp16b.py --mode nested --q 0.5 --seed 42
```
...
- curve 单元测试：15 passed（5.78 秒）。
  新增 test_exp16b_damping_reaches_hessian，明确断言配置中的
  Hessian damping=0.1、Fisher damping=0.001，
  并实际执行 Hessian refresh，检查所有 InverseLBFGS factors 的
  params['Kron_BFGS_H_epsilon']==0.1。
  原有 protocol 对照仅调整这一项预期值。
- 七组 CUDA tiny smoke：全部完成；每组 2 epochs、4 DP steps、
  最终 test accuracy=0.125、epsilon=0.9963923627。
  这些数值仅验证流程，不是正式实验结论。
- 结果完整性/有限数值及实际 CUDA 配置检查：
  `PYTHONDONTWRITEBYTECODE=1 conda run -n curve python exp16b/tests/check_smoke.py`，
  日志为 results/artifact_checks.log；七组均通过，
  且各组 config.json 均确认 Hessian damping=0.1、Fisher damping=0.001。
- Shell 语法检查：`conda run -n curve bash -n exp16b/run_all.sh`，通过。
- 未启动正式实验，exp16b/results/formal 尚未生成。

...
def spectrum(matrix, damping=DAMPING):
...
def build(model, x, y, kind, a_power=A_POWER, damping=DAMPING):
    if kind == 'identity':
    ...
    diagnostics = {'A': spectrum(factor['A'], damping), 'calibration_batches': 1,
    ...
    if kind == 'a_only':
        ...
        diagnostics['G'] = spectrum(factor['G'], damping)

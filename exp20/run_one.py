...
from exp20 import config as cfg
from exp20.methods import (synthetic_cache, build_from_cache, GhostNorm,
    ghost_aggregate, noise_and_step, noise_and_normalize)
from exp20.profiling import timestamp, timed, phase_start, phase_end, EventProfiler


def initialize(seed, device):
    # SimpleCNN initializes on CPU; leave unrelated CUDA generators untouched.
...
def run(method, seed, smoke, output):
    ...
    for epoch in range(1, epochs+1):
        ...
        stats = {name: 0. for name in ('ghost_first_pass_seconds',
        ...
        for x, y in loader:
            x, y = x.to(device), y.to(device)
            loss, norms, factors, layer_sq, phases = ghost_aggregate(model, hooks, x, y, train_profiler)
        ...
        for n, values in shares.items():
            ...
            layer_rows.append(dict(p=method, seed=seed, epoch=epoch, layer=n,
        ...
        rows.append(dict(p=method, seed=seed, epoch=epoch, train_loss=loss_sum/len(norm),
    ...
    configuration.update(smoke=smoke, epochs=epochs, noise_multiplier=sigma, p=method, seed=seed,
        synthetic_batches=1 if smoke else 10, total_steps=steps, drop_last=True,
        action_path_warmup=True,
...
def main():
    ...
    parser.add_argument('--p', type=float, choices=cfg.POWERS, required=True)
    ...
    with runtime('cuda:0'):
        run(args.p, args.seed, args.smoke, output)

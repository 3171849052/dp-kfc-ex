...
from exp16c.preconditioner import Preconditioner


def configuration(q, seed=42, smoke=False, mode="nested_ag"):
    extra = yaml.safe_load((ROOT / 'exp16c/configs/mnist.yaml').read_text())
    c = yaml.safe_load((ROOT / extra['base']).read_text())
    for section in ('data', 'synthetic', 'kfac'):
    ...
    c['data']['root'] = str(ROOT / 'exp16c/data')
    c['algorithm'] = 'synthetic_two_stage'
    c['output']['root'] = str(ROOT / 'exp16c/results')
    if smoke:
        c['data']['batch_size'] = 4
...
def run(q, seed=42, smoke=False, mode="nested_ag"):
    c = configuration(q, seed, smoke, mode)
    directory = ROOT / 'exp16c/results' / ('smoke' if smoke else 'formal') / f'{mode}_q{q:g}_seed{seed}'
...
CASES = [(mode, q) for q in (.25, .5)
                                      for mode in ('fisher', 'nested_ag', 'nested_a')]


def analyze(smoke=False):
    directory = ROOT / 'exp16c/results' / ('smoke' if smoke else 'formal')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--q', type=float, choices=[.25, .5])
    parser.add_argument('--mode', choices=['fisher', 'nested_ag', 'nested_a'], default='nested_ag')

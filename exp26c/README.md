# Exp26c: Full DP-KFC high-damping sweep

延伸 Exp26b 的单变量 damping sweep，判断高 damping 下 utility 是否继续改善，并寻找 turnover。
直接调用当前 `scripts/paper/exp_distilbert_sst2.py` 的 BK `run_one`，沿用 Exp26b 的实验包装方式，不修改 reference。

- Damping：`[0.1, 0.2, 0.5, 1.0, 2.0]`，共 5 个独立完整 run；0.1 是 Exp26b anchor，在本实验中独立重跑。
...
每个 run 在 `results/full_synthetic_bk_damping0.1_eps3_seed42.csv` 等独立文件中保留 3 个 epoch 行。
...
Accuracy 图加入 Exp26 同一工作点（C=2、LR=5e-4）的 DP-Adam 水平参考线 `0.8669724771`；norm-tail 图加入 p90 `261.4145`、p99 `766.1273` 水平参考线，仅用于分析，不重新训练 DP-Adam。
`ranking.csv` 额外包含 `p90_ratio_to_dp_adam` 和 `p99_ratio_to_dp_adam`，用于比较 tail 尺度。Loss/clip 图不添加未提供的 DP-Adam 参考值。

轻量检查：

```bash
conda run -n curve python -m py_compile exp26c/config.py exp26c/run.py exp26c/analyze.py
...
```bash
conda run -n curve bash exp26c/run_all.sh
```

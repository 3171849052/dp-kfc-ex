# Exp32: ImageNet CNN 全参数 DP 微调

测试 ImageNet 预训练的非 Transformer CNN 在 CIFAR-10 上，synthetic DP-KFC / DP-KFC-A 是否优于 DP-Adam。仅实现与轻量验证，尚未运行正式训练或产生准确率结论。

## 固定协议

- `torchvision.models.squeezenet1_1(weights=SqueezeNet1_1_Weights.IMAGENET1K_V1)`；seed=42，classifier dropout=0，seeded `Conv2d(512,10,1)`，全部 727,626 个参数可训练。
...
```bash
conda run --no-capture-output -n curve bash exp32/run_all.sh
```

启动器先检查目标 run 目录不存在，再准备权重和 CIFAR-10；已有任何正式 run 目录即报错。无自动 resume、跳过、fallback 或 batch size 下调。worker 失败时启动器返回失败，不生成不完整汇总。
...
所有新增文件只在 exp32 内。HF/Torch/CUDA/Triton/Inductor/matplotlib/XDG/TMP 缓存位于 `.cache/`，禁用 Python 字节码写入；数据下载到 `data/`；日志在 `logs/`。
...
在 curve 环境通过语法/import、正式 grid=7、GPU 分配=2/2/2/1、真实 ImageNet 权重加载、224×224 单样本 forward、10 类输出、dropout=0、26 层 Conv2d 覆盖、727,626 个 trainable 参数、Adam 类型与超参数检查。微型两层 CNN 的三种方法均通过逐样本 autograd oracle 对 transformed norm 与裁剪聚合梯度的核对，并完成一次带噪 Adam smoke step。日志见 `logs/checks.log`。

未启动正式训练，未验证 batch=256 的完整 GPU 显存占用；不会自动降低 batch size。

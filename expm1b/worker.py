"""Execute one fresh ExpM1b formal run in one CUDA process."""
...
from expm1b.bk import BKClipper
from expm1b.geometry import build_factors, geometry_rows
from expm1b.mechanism import Shape, add_noise_and_step


def _task_module(task: str):
    if task == "mnist":
        from expm1b import mnist

        return mnist
    assert task == "vit"
    from expm1b import vit
    ...
    for epoch in range(1, protocol.epochs + 1):
        ...
        if spec.method != "dp_sgd":
            source_batches = (
                ...
                else module.pink_calibration(spec.seed, epoch, device, need_g=need_g)
            )
            _synchronize(device)
        ...
        row: dict[str, object] = {
            ...
            "realized_batch_size_sum": sample_count,
            "realized_batch_size_mean": float(sample_count / logical_steps),
